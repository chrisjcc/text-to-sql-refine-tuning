...
def mock_torch_and_transformers():
    ...
    with (
        patch("src.inference.inference_engine.torch") as mock_torch,
...
def temp_model_dir():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        config_path.write_text(json.dumps({"base_model_name_or_path": "test-model"}))
        yield tmpdir
...
class TestInferenceEngine:
    ...
    def test_single_sql_generation(
        self, temp_model_dir, mock_environment, mock_parser, mock_torch_and_transformers
    ):
    ...
    def test_model_loading_peft(self, temp_model_dir, mock_torch_and_transformers):
        ...
        with (
            ...
            _engine = SQLInferenceEngine(  # noqa: F841
                model_path=temp_model_dir, base_model_name="test-model"
            )
    ...
    def test_sql_parsing(self, temp_model_dir, mock_environment, mock_torch_and_transformers):
        ...
        mock_tokenizer.decode.return_value = (
            "Test prompt SELECT id, name FROM users WHERE active = 1"
        )
        mock_model.generate.return_value = [[1, 2, 3, 4, 5]]
    ...
    def test_evaluation_on_dataset(
        self, temp_model_dir, mock_environment, mock_parser, mock_torch_and_transformers
    ):
